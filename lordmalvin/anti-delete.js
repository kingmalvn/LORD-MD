...
zokou({
  ...
  if (arg[0]) {
    ...
    if (action === "on") {
      ...
      await zk.sendMessage(origineMessage, "commande anti-delete is activated.");
      return;
    } else if (action === "off") {
      antiDeleteActive = false;
      await zk.sendMessage(origineMessage, "La commande anti-delete is deactivated.");
      return;
    }
  }
  ...
  if (!antiDeleteActive) {
    await zk.sendMessage(origineMessage, "La commande anti-delete is actually déactivée.");
    return;
  }
  ...
  if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0 && (conf.ADM).toLowerCase() === 'yes') {
    ...
    try {
      ...
      const caption = ` Anti-delete-message by 𝚖𝚊𝚕𝚟𝚒𝚗 tech\nMessage de @${senderId}`;
